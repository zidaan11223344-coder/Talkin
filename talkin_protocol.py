...
load_dotenv()
try:
    import talkin_settings as _settings
    for _name in (
        "BOT_ID", "BOT_PWD", "BOT_MASTER", "GROUP_TO_JOIN", "API_BASE_URL",
        "HOST_HEADER", "WS_HOSTS", "SOCKET_PORT", "WS_PATHS", "REFERRER_URL",
        "LANGUAGE", "SDK", "DEBUG", "RAW_DIAGNOSTIC", "ACK_ROOM_EVENTS", "AUTO_HELP",
    ):
        _value = getattr(_settings, _name, "")
...
BOT_MASTER = os.getenv("BOT_MASTER", "").strip() or BOT_ID
...
# Talkin accepts the generic client values when Android runtime information is
# unavailable (for example on a normal Linux/Python installation).
SDK = os.getenv("SDK", "").strip() or _ANDROID_SDK or "35"
...
_MANUFACTURER = _MANUFACTURER or "Android"
_MODEL = _MODEL or "TalkinClient"
...
class TalkinBot:
    ...
    def send_room_media(self, room: str, media_url: str, media_type: str = "image",
        ...
        """Send a public image or voice URL as a Talkin room message."""
        kind = str(media_type or "image").strip().lower()
        if kind in ("audio", "sound", "voice"):
            kind = "voice"
        elif kind in ("photo", "picture", "png", "jpg", "jpeg"):
            kind = "image"
        self.send_query(encode_query(
            "room_message", type_=kind, room=room, body=caption or "",
            url=str(media_url), length=str(duration_ms or "")
        ))
