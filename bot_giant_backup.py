...
TALKIN_MODE = os.environ.get("TALKIN_MODE", "0") == "1"
if missing and TALKIN_MODE:
    C["supabase_url"] = ""
    C["supabase_key"] = ""
    C["username"] = os.environ.get("BOT_ID", "talkin-bot")
    C["password"] = os.environ.get("BOT_PWD", "")
    missing = []
if missing:
...
PASSWORD = C["password"]
OWNER = (os.environ.get("BOT_MASTER") or C.get("owner_username") or USERNAME).strip().lower()
...
sb = None if TALKIN_MODE else create_supabase_client(C["supabase_url"], C["supabase_key"])
...
async def send_gift_command(rid, sender_uid, sender_name, raw_text):
    ...
    if TALKIN_MODE:
        receiver = {"id": receiver_name, "username": receiver_name}
    else:
    ...
    async with GIFT_POINTS_LOCK:
        ...
        unlimited_master = str(sender_name or "").strip().lower() == OWNER
        ...
        sender_data["points"] = (balance - cost) if not unlimited_master else 10**12
