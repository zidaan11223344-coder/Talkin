...
from media_features import handle as handle_media_feature
...
BOT_MASTER = os.getenv("BOT_MASTER", BOT_ID).strip()
...
class TalkinBot:
    ...
    def send_room_media(self, room: str, caption: str, media_url: str,
                        media_type: str = "image", duration_ms: int = 0):
        """Send an image/voice URL as a native Talkin room message."""
        media_type = str(media_type or "image").strip().lower()
        if media_type not in ("image", "voice", "audio"):
            raise ValueError("Unsupported Talkin media type")
        self.send_query(encode_query(
            "room_message", type_=media_type, room=room,
            body=caption or "", url=str(media_url),
            long_value=max(0, int(duration_ms or 0)),
        ))
    ...
    def handle_room_event(self, result):
        ...
        # Transferred music, games and gifts command layer. It uses the
        # target bot's native room sender and leaves moderation intact.
        try:
            if handle_media_feature(self, room, frm, body):
                return
        except Exception as e:
            self.log("[MEDIA] command failed:", repr(e))
            try:
                self.send_room_text(room, "❌ حدث خطأ مؤقت أثناء تنفيذ الأمر.")
            except Exception:
                pass
        ...
        try:
            ...
            if result.get("chat_message"):
                ...
                try:
                    ...
                    if body and body.lower().startswith(("دخول ", "join ", "ادخل ", "enter ")):
    ...
    def start(self):
        ...
        try:
            ...
            self.log("[MEDIA] media server unavailable:", repr(e))
        if not BOT_ID or not BOT_PWD:
            raise SystemExit("Set BOT_ID and BOT_PWD in .env first. Use private command: دخول اسم_الغرفة")
        while not self.stop_event.is_set():
            try:
                self.run_once()
            except Exception as e:
